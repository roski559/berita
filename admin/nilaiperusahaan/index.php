...
<div class="row">
				...
				<form method="POST" enctype="multipart/form-data">
					<div class="row">
						<div class="col-lg-12 mt-3">
							...
							<input type="file" name="file" class="form-control" value="<?= $data['image'] ?>">
	...
	<div class="col-lg-4 col-xs-12">
		<div class="card card-primary">
			...
			<div class="card-body">
				<img src="../assets/file/nilaiperusahaan/<?= $data['image'] ?>" style="width: 100%;height: 30vh;">
			</div>
		</div>
	</div>
	...
	if(isset($_POST['submit'])) {
		...
		if(!empty($gambar)) {
			if(in_array($ekstensi, $ekstensi_boleh) === true) {
				if($ukuran < 5000000) {
					move_uploaded_file($file_tmp, '../assets/file/nilaiperusahaan/'. $gambar);
					$sql = mysqli_query($con, "UPDATE tbl_nilaiperusahaan SET image='$gambar', isi='$isi'");
					echo "<script>alert('Data Berhasil Di ubah!')</script>";
					echo "<script>window.location.href='index.php?page=nilaiperusahaan'</script>";
				}
			} 
		} else {
			$gambar = $data['image'];
