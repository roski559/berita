<?php 
	...
	$sinopsis = $_SESSION['pengguna'];

	$sql = mysqli_query($con, "SELECT * FROM tbl_posts WHERE kategori='berita' AND sinopsis='$sinopsis' ORDER BY date DESC");
			...
			<div class="card-body">
				...
				<?php $no = 1; foreach($sql as $data): ?>
					<tr>
						...
						<td class="text-center">
							<img src="../assets/file/thumbnail/<?= $data['thumbnail'] ?>" width="80" height="50">
						</td>
						<td><?= $data['date'] ?></td>
						<td><?= $data['kategori'] ?></td>
						<td><?= $data['sinopsis'] ?></td>
