<?php 
	...
	$sql = mysqli_query($con, "SELECT * FROM tbl_sejarah");
...
<div class="row">
	...
	<div class="col-lg-4 col-xs-12">
		<div class="card card-primary">
			<div class="card-header">
				<h5>Gambar Data Sejarah</h5>
			</div>
			<div class="card-body">
				<img src="../assets/file/sejarah/<?= $data['gambar'] ?>" style="width: 100%;height: 30vh;">
			</div>
		</div>
	</div>
	...
	if(isset($_POST['submit'])) {
		...
		if(!empty($gambar)) {
			if(in_array($ekstensi, $ekstensi_boleh) === true) {
				if($ukuran < 5000000) {
					move_uploaded_file($file_tmp, '../assets/file/sejarah/'. $gambar);
					$sql = mysqli_query($con, "UPDATE tbl_sejarah SET gambar='$gambar', isi='$isi'");
					echo "<script>alert('Data Berhasil Di ubah!')</script>";
					echo "<script>window.location.href='index.php?page=sejarah'</script>";
				}
			} 
		} else {
			$gambar = $data['gambar'];
